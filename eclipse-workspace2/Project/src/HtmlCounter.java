...
public class HtmlCounter extends WordCounter{
	private String urlStr;
    private String content;
    
    public HtmlCounter(String urlStr){
    	super(urlStr);
    }
    ...
    public void match() throws IOException{	
		...
		while((indexOfOpen = content.indexOf("<",indexOfOpen))!=-1){
			...
			if((indexOfSpace=fullTag.indexOf(" "))==-1){
				...
				//tagName="";
				//for(int i=1;i<fullTag.length();i++) {
				//if(fullTag.substring(i,i+1).equals(" ")) {
				//		break;
				//	}
				//else {
				//		tagName+=fullTag.substring(i,i+1);
				//	}
				//}
    ...
    public int countTypeKeyword(String typeKeyword) {
    	int count=0;
    	if(content.contains(typeKeyword)) {
    		count++;
    	}
    	return count;
    }
    
//    public int calTypeScore(String typeKeyword) {
//    	int score=typeKeyword.weight*countTypeKeyword(typeKeyword);
//    }
}
